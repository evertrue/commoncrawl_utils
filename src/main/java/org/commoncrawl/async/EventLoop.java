...
public class EventLoop implements Runnable {
	...
	public void stop() {
		...
		if (Thread.currentThread() != _eventThread) {
			try {
				LOG.info("Waiting for Event Thread to DIE TID:" + Thread.currentThread().getId());
				_eventThread.join();
				_eventThread = null;
				LOG.info("Event Thread DEAD. Exiting EventLoop TID:" + Thread.currentThread().getId());
	...
	*/
	...
	public boolean waitForIO(long waitTime) throws IOException {
		...
		while (!_shutdown) {
			...
			if (waitTime > 0) {
				if (nextFireTime != 0) {
					waitTime = Math.max(1, Math.min(nextFireTime - System.currentTimeMillis(), waitTime));
				}
	...
	public void run() {
		...
		try {
			...
			LOG.fatal("Unhandled Exception in Event Loop:" + StringUtils.stringifyException(e));
			System.out.println("Unhandled Exception in Event Loop:" + StringUtils.stringifyException(e));
		}
	...
	public <ResultType> void queueAsyncCallbackWithResult(final CallbackWithResult<ResultType> callback,
