...
public class MergeSortSpillWriterUnitTest {
	...
	static abstract class BaseTest {
		...
		public void runTest() throws IOException {
			...
			RawDataSpillWriter<IntWritable, Text> validatingSpillWriter = new RawDataSpillWriter<IntWritable, Text>() {
				...
				public void close() throws IOException {
					...
					if (spilledKeyCount != index.length) {
						throw new IOException("Spilled Key Count:" + spilledKeyCount + " Excpected Key Count:"
								+ index.length);
					}
				}
				...
				@Override
				public void spillRawRecord(byte[] keyData, int keyOffset, int keyLength, byte[] valueData,
			...
			RawKeyValueComparator<IntWritable, Text> comparator = new RawKeyValueComparator<IntWritable, Text>() {
				...
				public int compareRaw(byte[] key1Data, int key1Offset, int key1Length, byte[] key2Data, int key2Offset,
						int key2Length, byte[] value1Data, int value1Offset, int value1Length, byte[] value2Data,
			...
			MergeSortSpillWriter<IntWritable, Text> merger = constructMerger(conf, validatingSpillWriter,
					FileSystem.getLocal(conf), new Path("/tmp"), comparator, IntWritable.class, Text.class);
		...
		protected abstract MergeSortSpillWriter<IntWritable, Text> constructMerger(Configuration conf,
				...
				RawKeyValueComparator<IntWritable, Text> comparator, Class keyClass, Class valueClass)
				throws IOException;
	...
	public static class BasicTest extends BaseTest {
		...
		protected MergeSortSpillWriter<IntWritable, Text> constructMerger(Configuration conf,
			...
			return new MergeSortSpillWriter<IntWritable, Text>(conf, writer, tempFileSystem, tempFilePath, null,
					comparator, keyClass, valueClass, null, null);
		}
	}
		...
		protected MergeSortSpillWriter<IntWritable, Text> constructMerger(Configuration conf,
			...
			return new MergeSortSpillWriter<IntWritable, Text>(conf, writer, tempFileSystem, tempFilePath,
					new OptimizedKeyGeneratorAndComparator<IntWritable, Text>() {
						...
						@Override
						public int compareOptimizedBufferKeys(byte[] key1Data, int key1Offset, int key1Length,
								byte[] key2Data, int key2Offset, int key2Length) throws IOException {
	...
	public static class BasicOptimizedWithBufferOnlyTest extends BaseTest {
		...
		public BasicOptimizedWithBufferOnlyTest(int keySetSize, int indexBufferSize, int dataBufferSize,
				int spillBufferSize) {
