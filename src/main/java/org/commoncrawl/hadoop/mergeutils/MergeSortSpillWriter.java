...
public class MergeSortSpillWriter<KeyType extends WritableComparable, ValueType extends Writable> implements
	 ...
	 * @throws IOException
	 */
	public MergeSortSpillWriter(Configuration conf, RawDataSpillWriter<KeyType, ValueType> outputSpillWriter,
			FileSystem tempDataFileSystem, Path tempDirectoryPath,
			SpillValueCombiner<KeyType, ValueType> optionalCombiner,
			RawKeyValueComparator<KeyType, ValueType> comparator, Class<KeyType> keyClass, Class<ValueType> valueClass,
			CompressionCodec compressionCodec, Reporter reporter) throws IOException {

		init(conf, outputSpillWriter, tempDataFileSystem, tempDirectoryPath, comparator, null, optionalCombiner,
				keyClass, valueClass, compressionCodec, reporter);
	}
	...
	public MergeSortSpillWriter(Configuration conf, RawDataSpillWriter<KeyType, ValueType> outputSpillWriter,
			FileSystem tempDataFileSystem, Path tempDirectoryPath,
			OptimizedKeyGeneratorAndComparator<KeyType, ValueType> keyGenerator, Class<KeyType> keyClass,
			Class<ValueType> valueClass, CompressionCodec compressionCodec, Reporter reporter) throws IOException {

		init(conf, outputSpillWriter, tempDataFileSystem, tempDirectoryPath, null, keyGenerator, null, keyClass,
				valueClass, compressionCodec, reporter);
	}

	private void init(Configuration conf, RawDataSpillWriter<KeyType, ValueType> outputSpillWriter,
			FileSystem tempDataFileSystem, Path tempDirectoryPath,
			RawKeyValueComparator<KeyType, ValueType> comparator,
			OptimizedKeyGeneratorAndComparator<KeyType, ValueType> optionalGenerator,
			SpillValueCombiner<KeyType, ValueType> optCombiner, Class<KeyType> keyClass, Class<ValueType> valueClass,
			CompressionCodec compressionCodec, Reporter reporter) throws IOException {
		...
		_temporaryDirectoryPath = new Path(tempDirectoryPath, Long.toString(Thread.currentThread().getId()) + "-"
	...
	public void close() throws IOException {
		...
		if (_mergeSegements.size() != 0) {
			...
			if (_optimizedKeyGenerator != null) {
				merger = new SequenceFileMerger<KeyType, ValueType>(_tempDataFileSystem, _conf, _mergeSegements,
						_outputSpillWriter, _keyClass, _valueClass, _optimizedKeyGenerator);
			} else {
				merger = new SequenceFileMerger<KeyType, ValueType>(_tempDataFileSystem, _conf, _mergeSegements,
						_outputSpillWriter, _keyClass, _valueClass, _optCombiner, _comparator);
			}
	...
	private void sortAndSpill(RawDataSpillWriter<KeyType, ValueType> optionalWriter) throws IOException {
		...
		if (optionalWriter != null) {
			...
			spillWriter = new SequenceFileSpillWriter<KeyType, ValueType>(_tempDataFileSystem, _conf, spillFilePath,
					_keyClass, _valueClass, null, _compressionCodec, (short) _conf.getInt(
							SPILL_FILE_REPLICATION_FACTOR_PARAM, DEFAULT_REPLICATION_FACTOR));
		}
			...
			for (i = 0; i < _spillItemCount; ++i) {
				try {
					...
					if (directSpillOutput || _optimizedKeyGenerator == null) {
						spillWriter.spillRawRecord(bufferAsBytes, keyPos, keyLen, bufferAsBytes, valuePosition,
								valueLen);
					}
					// otherwise ... in the optimized key case ... 
					else {
						spillWriter.spillRawRecord(bufferAsBytes, dataOffset, keyLen + _optimizedKey.getHeaderSize()
								+ 4, bufferAsBytes, valuePosition, valueLen + optimizedBufferLen);
					}
					// increment progress... 
					if (i % 100000 == 0) {
					...
					LOG.error("Error in Iteration. " + " DataOffset:" + dataOffset + " index:" + i + " keyLen:"
							+ keyLen + " keyPos:" + keyPos + " valueLen:" + valueLen + " valuePos:" + valuePosition
							+ " spillDataBufferSize:" + _spillDataBuffer.capacity() + " spillDataPosition:"
							+ _spillDataBuffer.position() + " spillDataRemaining:" + _spillDataBuffer.remaining());
	...
	static final int getIntB(byte[] bb, int offset) {
		return (int) ((((bb[offset + 0] & 0xff) << 24) | ((bb[offset + 1] & 0xff) << 16)
				| ((bb[offset + 2] & 0xff) << 8) | ((bb[offset + 3] & 0xff) << 0)));
	}
	...
	private static final int compareUsingRawComparator(RawKeyValueComparator comparator, byte[] keyValueData1,
		...
		return comparator.compareRaw(keyValueData1, key1Offset, key1Length, keyValueData2, key2Offset, key2Length,
				keyValueData1, value1Offset, value1Length, keyValueData2, value2Offset, value2Length);
	}
	...
	private final int med3UsingOptimizedComparatorWithLongs(int x[], int a, int b, int c) throws IOException {
		...
		return (aValue < bValue ? (bValue < cValue ? b : aValue < cValue ? c : a) : (bValue > cValue ? b
				: aValue > cValue ? c : a));
	}
	...
	private final int med3UsingRawComparator(byte[] spillData, int x[], int a, int b, int c) throws IOException {
		...
		return (compareUsingRawComparator(_comparator, spillData, x[a], spillData, x[b]) < 0 ? // x[a] < x[b] ?
	...
	private final int med3UsingOptimizedComparatorWithLongsAndBuffer(byte[] spillData, int x[], int a, int b, int c)
		...
		return (compareUsingOptimizedRawLongAndBufferValues(spillData, x[a], x[b]) < 0 ? // x[a] < x[b] ?
	...
	private final int med3UsingOptimizedComparatorWithBuffer(byte[] spillData, int x[], int a, int b, int c)
		...
		return (compareUsingOptimizedRawBufferValues(spillData, x[a], x[b]) < 0 ? // x[a] < x[b] ?
		((compareUsingOptimizedRawBufferValues(spillData, x[b], x[c]) < 0) ? b : (compareUsingOptimizedRawBufferValues(
				spillData, x[a], x[c]) < 0) ? c : a) : // (x[b] < x[c] ? b : x[a] < x[c] ? c : a) : 
	...
	private void sortUsingRawComparator(byte[] dataBytes, int x[], int off, int len) throws IOException {
		...
		if (len < 7) {
			for (int i = off; i < len + off; i++)
				...
				for (int j = i; j > off
						&& compareUsingRawComparator(_comparator, spillData, x[j - 1], spillData, x[j]) > 0; j--)
					swap(x, j, j - 1);
			return;
		}
	...
	private final int compareUsingOptimizedRawLongAndBufferValues(byte[] dataAsBytes, int lValueDataOffset,
		...
		int result = (lValue > rValue) ? 1 : (lValue < rValue) ? -1 : 0;
		if (result == 0) {
			return _optimizedKeyGenerator.compareOptimizedBufferKeys(dataAsBytes, lValueDataOffset + buffer1Offset,
					buffer1Len, dataAsBytes, rValueDataOffset + buffer2Offset, buffer2Len);
		}
		return result;
	}

	private final int compareUsingOptimizedRawBufferValues(byte[] dataAsBytes, int lValueDataOffset,
			int rValueDataOffset) throws IOException {
