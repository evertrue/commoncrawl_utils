...
public final class NIODNSLocalResolver implements NIODNSResolver {
						...
						if (!isCancelled()) {
							...
							if (qResult != null) {
								...
								if (client != null) {
									...
									if (qResult.success()) {
										client.AddressResolutionSuccess(qResult._source, qResult.getHostName(),
												qResult.getCName(), qResult.getAddress(), qResult.getTTL());
									} else {
										client.AddressResolutionFailure(qResult._source, qResult.getHostName(),
												qResult.getStatus(), qResult.getErrorDescription());
									}
		...
		if (true) {
			...
			try {
				...
				try {
					...
					LOG.error("Reverse DNS Resolution for:" + address + " threw IOException:"
	...
	public NIODNSQueryResult doDNSQuery(NIODNSQueryClient client, String hostName, boolean useTCP, boolean noCache,
						...
						if (address != null) {
							...
							if (!noCache)
								_dnsCache.cacheIPAddressForHost(hostName,
										IPAddressUtils.IPV4AddressToInteger(address.getAddress()), expireTime, cname);
						}
					}
		...
		if (!resultViaCache && result.getStatus() == Status.SERVER_FAILURE) {
			if (result.getErrorDescription().equals("NXDOMAIN")) {
				_badHostCache.cacheIPAddressForHost(hostName, 0, System.currentTimeMillis()
	...
	private static class DNSQuery implements Callable<NIODNSQueryResult> {
		 ...
		 */
		public DNSQuery(NIODNSQueryClient client, NIODNSLocalResolver resolver, String hostName, int flags,
				int timeoutValue) {
