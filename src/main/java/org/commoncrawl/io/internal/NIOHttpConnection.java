...
public final class NIOHttpConnection implements NIOClientSocketListener, NIODNSQueryClient {
	...
	public enum State {
		IDLE, AWAITING_RESOLUTION, AWAITING_CONNECT, SENDING_REQUEST, RECEIVING_HEADERS, PARSING_HEADERS, RECEIVING_CONTENT, DONE, ERROR
	};
	...
	public NIOHttpConnection(URL theURL, NIOSocketSelector selector, NIODNSResolver resolver,
			NIOHttpCookieStore cookieStore) throws IOException {
		_url = theURL;
	...
	public final NIOClientSocket getSocket() {
		return _socket;
	}
	...
	private void startConnect(InetSocketAddress addressToConnectTo) {
		...
		if (_socket != null && _socket.isOpen()) {
			...
			try {
				...
				LOG.error("Socket Connect via Address Resolution Success for host:" + _url.getHost()
		...
		if (_foundStatusLine) {
			if (accumulateHeaders(HTTP_HEADER_SIZE_MAX)) {
				...
				Reader reader = new InputStreamReader(new ByteArrayInputStream(
						_incomingAccumulationBuffer.toByteArray()), Charset.forName("UTF-8"));
