					...
					if (theSocket != null && theSocket.getListener() != null) {
						...
						if (selectionKey.isConnectable()) {
							...
							try {
								...
								((NIOClientSocketListener) theSocket.getListener()).Excepted(
										(NIOClientSocket) theSocket, e);
								theSocket.close();
								disconnectException = e;
								//KILL THE SERVER 
								//throw e;
							...
							if (!connected) {
								...
								((NIOClientSocketListener) theSocket.getListener()).Disconnected(
										(NIOClientSocket) theSocket, disconnectException);
						...
						if (selectionKey.isValid() && selectionKey.isReadable()) {
							...
							try {
								...
								if (bytesRead == -1) {
									...
									((NIOClientSocketListener) theSocket.getListener()).Disconnected(
											(NIOClientSocket) theSocket, null);
									// close the socket ... 
									theSocket.close();
								}
							} catch (RuntimeException e) {
