...
public final class ARCFileReader extends InflaterInputStream {
	...
	public ARCFileReader(final InputStream source) throws IOException {
		super(new CustomPushbackInputStream(new CountingInputStream(source), _blockSize), new Inflater(true),
				_blockSize);
		readARCHeader();
	}
	...
	public static class ArcFileBuilder {
		...
		// match count
		enum CRLFMatchState {
			CRLFMatchState_NoMatch, CRLFMatchState_FirstCR, CRLFMatchState_FirstLF, CRLFMatchState_SecondCR,
		}
		...
		private static InputStreamReader readerFromScanBufferList(LinkedList<ByteBuffer> buffers, Charset charset)
				throws IOException {
		...
		static final Pattern metadataLinePattern = Pattern
				.compile("^(.*)\\s([0-9\\.]+)\\s([0-9]+)\\s([^\\s]+)\\s([0-9]+)$");
		...
		private final void processHeaders() throws IOException {
			...
			if (_payloadLength == 0) {
				LOG.warn("Invalid Payload Size Detected Key:" + _keyOut.toString() + " RecordLen:" + _recordLen
						+ " HeaderLen:" + rawHeaderSize + " Headers:\n" + _headers.toString());
		...
		public final void finish() throws IOException {
			if (_state == State.ReadingContent) {
				...
				throw new IOException("ArcBuilder finish calledin Invalid State. State:" + _state + " ArcFile:"
						+ _keyOut + " Position:" + _streamPos + " Item URI:" + _keyOut);
			}
		}
	...
	static {
		...
		options.addOption(OptionBuilder.withArgName("conf").hasArg().withDescription("Config File Name").create("conf"));

		options.addOption(OptionBuilder.withArgName("file").hasArg().withDescription("ARC File Path").isRequired()
				.create("file"));

		options.addOption(OptionBuilder.withArgName("awsAccessKey").hasArg().withDescription("AWS Access Key")
				.create("awsAccessKey"));

		options.addOption(OptionBuilder.withArgName("awsSecret").hasArg().withDescription("AWS Secret")
				.create("awsSecret"));
