...
public class ArcFileItemUtils {
	...
	public static void bytesWritableToArcFileItem(Text key, BytesWritable rawArcPayload, ArcFileItem arcFileItem)
			throws IOException {
		...
		try {
			arcFileItem.setTimestamp(TIMESTAMP14.parse(headers.findValue(Constants.ARCFileHeader_ARC_Timestamp))
					.getTime());
		} catch (Exception e) {
			LOG.error("Invalid Timestamp Encountered in Item Metdata. URL:" + arcFileItem.getUri() + " Timestamp:"
					+ headers.findValue(Constants.ARCFileHeader_ARC_Timestamp));
		}
