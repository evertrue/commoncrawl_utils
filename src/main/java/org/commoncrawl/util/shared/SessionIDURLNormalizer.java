	...
	public String normalize(String urlString, String scope) throws MalformedURLException {
...

		{
			...
			if (phpsessidIDX != -1) {
				...
				if (urlString.length() > charPosAfterPHPSessId) {
					...
					if (charAfterPHPSessId == '=' || charAfterPHPSessId == '.' || charAfterPHPSessId == '-'
							|| charAfterPHPSessId == '+' || charAfterPHPSessId == '/') {
						int idStart = charPosAfterPHPSessId + 1;
						int idEnd = idStart;
						while (idEnd != urlString.length()) {
							if (urlString.charAt(idEnd) == '&' || urlString.charAt(idEnd) == '.'
									|| urlString.charAt(idEnd) == '-' || urlString.charAt(idEnd) == '+'
		...
		if (dashdashIDX != -1) {
			...
			if (nextDashDashIDX != -1) {
				Matcher m = pattern0_2.matcher(urlString.substring(dashdashIDX + DASHDASH_SESSIONID.length(),
						nextDashDashIDX));
				if (m.matches()) {
...

		{
			...
			if (sidIDX != -1 && sidIDX != 0) {
				if (urlString.charAt(sidIDX - 1) == '/' || urlString.charAt(sidIDX - 1) == '?'
						|| urlString.charAt(sidIDX - 1) == '&' || urlString.charAt(sidIDX - 1) == '+') {
					matchingStr = SID;
				}
			}
			if (matchingStr == null) {
			...
			if (matchingStr != null) {
				{
					...
					if (urlString.length() > charPosAfterSessId) {
						...
						if (charAfterSessId == '=' || charAfterSessId == '.' || charAfterSessId == '-'
