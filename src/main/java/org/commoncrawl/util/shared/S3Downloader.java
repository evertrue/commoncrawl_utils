...
public class S3Downloader implements NIOHttpConnection.Listener {
	...
	private void downloadItem(S3DownloadItem item) {
		...
		try {
			...
			URL theURL = _callingFormat.getURL(false, S3Utils.DEFAULT_HOST, S3Utils.INSECURE_PORT, _s3BucketName,
					item.getKey(), null);
			...
			String canonicalString = S3Utils.makeCanonicalString("GET", _s3BucketName, item.getKey(), null,
					amazonHeaders);
	...
	private final void failDownload(S3DownloadItem item, NIOHttpConnection.ErrorType errorType,
			NIOHttpConnection theConnection, boolean potentiallyRetry) {
		...
		if (item != null) {
			if (potentiallyRetry && item.isDownloadRecoverable()) {
				...
				if (_callback != null) {
					_callback.downloadFailed(item.getId(), item.getKey(), "Failure Reason:" + errorType.toString()
	...
	public void HttpConnectionStateChanged(NIOHttpConnection theConnection, State oldState, State state) {
		...
		if (oldState == State.PARSING_HEADERS && state == State.RECEIVING_CONTENT) {
			...
			LOG.info("Content Length From Header for:" + theConnection.getURL() + " is:"
					+ headers.findValue("Content-Length"));
		}
		...
		if (state == State.RECEIVING_CONTENT) {
			...
			if (continueDownloading) {
				if (!isContinuation) {
					if (_callback != null) {
						continueDownloading = _callback.downloadStarting(item.getId(), item.getKey(),
	...
	public void HttpContentAvailable(NIOHttpConnection theConnection, NIOBufferList contentBuffer) {
		...
		if (item != null) {
			...
			if (_callback != null) {
				continueDownload = _callback
						.contentAvailable(theConnection, item.getId(), item.getKey(), contentBuffer);
			}
	...
	private static class S3DownloadItem {
		...
		public boolean isDownloadRecoverable() {
			...
			if (_lastErrorType == NIOHttpConnection.ErrorType.UNKNOWN
					|| _lastErrorType == NIOHttpConnection.ErrorType.IOEXCEPTION
					|| _lastErrorType == NIOHttpConnection.ErrorType.TIMEOUT) {
