	...
	public static String bestEffortDecodeBytes(NIOHttpHeaders headers, byte[] crawlData, int offset, int length)
		...
		if (urlMetadata._charset != null && crawlData != null) {
			if (urlMetadata._contentType != null && urlMetadata._contentType.equalsIgnoreCase("text/html")) {
				...
				String alternateCharset = CharsetUtils.sniffCharacterEncoding(crawlData, offset, length);
		...
		if (urlMetadata._charset == null && crawlData != null) {
			...
			if (urlMetadata._contentType != null && MimeTypeFilter.isTextType(urlMetadata._contentType)) {
