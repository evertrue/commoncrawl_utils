 ...
 *
 */
public class RPCChannel implements NIOClientSocketListener, Comparable<RPCChannel>, Channel {
	...
	private Map<Integer, OutgoingMessageContext<? extends RPCStruct, ? extends RPCStruct>> _requestMap = Collections
			.synchronizedMap(new HashMap<Integer, OutgoingMessageContext<? extends RPCStruct, ? extends RPCStruct>>());

	// list of requests that were already sent
	private LinkedList<OutgoingMessageContext<? extends RPCStruct, ? extends RPCStruct>> _sendQueue = new LinkedList<OutgoingMessageContext<? extends RPCStruct, ? extends RPCStruct>>();
	...
	RPCFrame.Decoder _decoder = new RPCFrame.Decoder(_inputStream);
	RPCFrame.Encoder _encoder = new RPCFrame.Encoder(_outputStream);
	 ...
	 * @throws IOException
	 */
	public RPCChannel(EventLoop eventLoop, ThreadPoolExecutor optionalThreadPool, InetSocketAddress localAddress,
			InetSocketAddress address, ConnectionCallback callback) throws IOException {
	...
	public synchronized int Readable(NIOClientSocket theSocket) throws IOException {
		...
		try {
			...
			LOG.error("IOException in Readable callback:" + CCStringUtils.stringifyException(e));
			e.printStackTrace();
			reconnect();
		}
		...
		while ((incoming = _decoder.getNextResponseFrame()) != null) {
			...
			if (associatedRequest != null) {
				...
				try {
					...
					associatedRequest.setStatus(OutgoingMessageContext.Status.values()[incoming._status]);
					...
					if (associatedRequest.getStatus() == OutgoingMessageContext.Status.Success) {
						...
						associatedRequest.getOutput().deserialize(new DataInputStream(incoming._payload),
								new BinaryProtocol());
					}
					// and if server error ...
					...
					LOG.error("IOException in readResponseFrame:" + CCStringUtils.stringifyException(e));
