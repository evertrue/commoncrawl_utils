...
public abstract class InProcessActor {
	...
	public static class IncomingMessage extends IncomingMessageContext {
		...
		public IncomingMessage(OutgoingMessageContext source, Channel channel, int requestId, RPCStruct input,
				RPCStruct output) {
	...
	private final Channel createChannel(final InProcessActor optionalSourceActor,
			final EventLoop optionalSourceEventLoop, final ThreadPoolExecutor optionalSourceExecutor) {
		...
		return new InProcessChannel() {
			...
			public void sendRequest(
					final OutgoingMessageContext<? extends RPCStruct, ? extends RPCStruct> originalMessage)
					throws RPCException {
				final IncomingMessage incomingMessage = new IncomingMessage(originalMessage, this,
						originalMessage.getRequestId(), (RPCStruct) originalMessage.getInput(),
						(RPCStruct) originalMessage.getOutput());
