...
public class RPCActorService {
	...
	private static class RPCActorInstance {
		...
		RPCActorInstance(ActorInterface.RPCSpecification spec, ActorInterface actorInterface,
	 ...
	 */
	public final void bindActor(RPCServerChannel channel, ActorInterface.RPCSpecification specification,
			ActorInterface actorInstance, ThreadPoolExecutor optionalThreadPool) {
		...
		synchronized (_channelMap) {
			...
			item._serviceMap.put(specification._name, new RPCActorInstance(specification, actorInstance,
					optionalThreadPool));
		}
	}
	...
	public final void dispatchWireRequest(final RPCServerChannel serverChannel, final RPCChannel source,
			final IncomingFrame frame) throws RPCException {
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				try {
					dispatcher.dispatch(dispatchInstance, serverChannel, frame._service, frame._method, payload,
							frame._requestId, source);
