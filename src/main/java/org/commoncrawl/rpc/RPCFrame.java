...
public class RPCFrame {
	...
	public static final class Encoder {
		...
		public synchronized int encodeResponse(IncomingMessageContext context) throws IOException {
			...
			headerStream.writeByteField(RPCFrame.MSG_HEADER_FIELD_TYPE, RPCFrame.MSG_TYPE.RESPONSE.ordinal());

			headerStream.writeIntField(RPCFrame.MSG_HEADER_FIELD_REQUEST_ID, context.getRequestId());

			headerStream.writeByteField(RPCFrame.MSG_HEADER_FIELD_STATUS, context.getStatus().ordinal());
		...
		public synchronized int encodeRequest(OutgoingMessageContext request) throws IOException {
			...
			headerStream.writeByteField(RPCFrame.MSG_HEADER_FIELD_TYPE, RPCFrame.MSG_TYPE.REQUEST.ordinal());

			headerStream.writeUTFField(RPCFrame.MSG_HEADER_FIELD_SERVICE, request.getServiceName());

			headerStream.writeUTFField(RPCFrame.MSG_HEADER_FIELD_METHOD, request.getMethodName());

			headerStream.writeIntField(RPCFrame.MSG_HEADER_FIELD_REQUEST_ID, request.getRequestId());
		...
		private synchronized int encode(HeaderOutputStream header, PayloadOutputStream payload) throws IOException {
			...
			return RPCFrame.FRAME_HEADER_SIZE + header.getLength() + payload.getLength();
		}
		...
		private IncomingFrame getNextFrame(boolean isRequestFrame) throws IOException {
			...
			try {
				...
				if (_lookAheadSize != FRAME_HEADER_SIZE && _stream.available() != 0) {
					// read header bytes ...
					_lookAheadSize += _stream
							.read(_lookAheadBuffer, _lookAheadSize, FRAME_HEADER_SIZE - _lookAheadSize);
				}
				...
				debugBuffer
						.append("LookAheadSize == FRAME_HEADER_SIZE:" + (_lookAheadSize == FRAME_HEADER_SIZE) + "\n");
				if (_lookAheadSize == FRAME_HEADER_SIZE) {
					if (_lookAheadBuffer[0] != SIGNATURE[0] || _lookAheadBuffer[1] != SIGNATURE[1]
							|| _lookAheadBuffer[2] != VERSION[0] || _lookAheadBuffer[3] != VERSION[1]) {
						...
						debugBuffer.append("  Exp:[" + SIGNATURE[0] + "][" + SIGNATURE[1] + "][" + VERSION[0] + "]["
								+ VERSION[1] + "]\n");
					} else {
