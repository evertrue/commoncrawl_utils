...
public abstract class QuerySlaveServer implements QueryServerSlave, QueryProgressCallback,
	...
	public abstract Query createQueryObjectGivenType(String queryObjectType, int shardId, RPCStruct queryDataObject)
			throws IOException;
	...
	public void doQuery(IncomingMessageContext<RemoteQueryInfo, QueryStatus> rpcContext) throws RPCException {
		...
		LOG.info(_fqName + " Adding Query Type:" + rpcContext.getInput().getQueryClassType() + "Id:"
				+ rpcContext.getInput().getCommonInfo().getQueryId() + " to Queue.");
		try {
			// extract object type 
			String queryObjectType = rpcContext.getInput().getQueryClassType();
			LOG.info(_fqName + " QueryId:" + rpcContext.getInput().getCommonInfo().getQueryId() + " ObjectType:"
					+ queryObjectType);
			// and data type 
			String queryDataType = rpcContext.getInput().getQueryDataClassType();
			LOG.info(_fqName + " QueryId:" + rpcContext.getInput().getCommonInfo().getQueryId() + " QueryDataType:"
					+ queryDataType);
			// allocate the object data type .. 
			RPCStruct queryData = createQueryDataObjectGivenType(queryDataType);
			LOG.info(_fqName + " QueryId:" + rpcContext.getInput().getCommonInfo().getQueryId()
					+ " DeSerializing Query Data");
			// allocate an input stream  
			DataInputBuffer inputStream = new DataInputBuffer();
			inputStream.reset(rpcContext.getInput().getQueryDataBuffer().getReadOnlyBytes(), 0, rpcContext.getInput()
					.getQueryDataBuffer().getCount());
			// and deserialize into the structure 
			queryData.deserialize(inputStream, new BinaryProtocol());
			LOG.info(_fqName + " QueryId:" + rpcContext.getInput().getCommonInfo().getQueryId()
					+ " Allocating QueryOp Object of type:" + queryObjectType);
			// allocate query object 
			Query queryObject = createQueryObjectGivenType(queryObjectType, rpcContext.getInput().getShardId(),
					queryData);
			LOG.info(_fqName + " QueryId:" + rpcContext.getInput().getCommonInfo().getQueryId()
					+ " Initializing QueryObject");
			// initialize query 
	...
	@Override
	public void initializeQuerySlave(final IncomingMessageContext<BaseConfig, SlaveStatus> messageContext)
			throws RPCException {

		_maxConcurrentQueries = getConfiguration().getInt(SLAVE_MAX_CONCURRENT_QUERIES_PARAM,
				SLAVE_MAX_CONCURRENT_QUERIES_DEFAULT);
	...
	public boolean updateProgress(Query theQueryObject, float percentComplete) {
		LOG.info(_fqName + " Update Progress Received for Query:" + theQueryObject.getQueryId() + "pctComplete:"
				+ percentComplete);
		...
		if (!_cancelling) {
			synchronized (_cancelledQueries) {
				...
				if (_cancelledQueries.contains(request.getQueryId())) {
					...
					LOG.info(_fqName + " Query Seems to have been cancelled. Explicitly cancelling Query:"
							+ request.getQueryId());
					// override status 
	...
	private File getTempDirForShardedQuery(Query query) {
		return new File(getLocalQueryTempDir(), Long.toString(query.getQueryId()) + "-"
				+ query.getRemoteQueryInfo().getShardId());
	}
	...
	private void activateQuery(Query queryObject) {
		...
		try {
			...
			queryObject.runShardedQuery(_fqName, getRemoteFileSystem(), getConfiguration(), getEventLoop(), this,
					getTempDirForShardedQuery(queryObject), this, this);
			// and update the status.
			updateSlaveStatusForQueryObject(queryObject);
		} catch (IOException e) {
			LOG.info(_fqName + " Query Activation for Query:" + queryObject.getQueryId() + " Failed with Exception:"
	...
	private void updateSlaveStatusForQueryObject(Query theQueryObject) {
		...
		synchronized (_slaveStatus) {
			for (QueryStatus status : _slaveStatus.getQueryStatus()) {
				...
				if (status.getQueryId() == queryId && status.getShardId() == shardId) {
					targetStatus = status;
					break;
				}
			}
