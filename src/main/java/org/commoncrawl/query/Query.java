...
public abstract class Query<DataType extends RPCStruct, ResultKeyType extends WritableComparable, ResultValueType extends Writable> {
	...
	public void runShardedQuery(final String fqHostName, final FileSystem remoteFileSystem, final Configuration conf,
			final EventLoop eventLoop, final QuerySlaveServer server, final File queryTempDir,
		...
		_queryThread = new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					Class<? extends CompressionCodec> codecClass = conf.getClass("mapred.output.compression.codec",
							null, CompressionCodec.class);
					if (codecClass != null) {
						codec = ReflectionUtils.newInstance(codecClass, conf);
					}

					LOG.info(fqHostName + " Creating SpillWriter with output file:" + shardDataFile + " for Query:"
							+ getQueryId() + " Codec is:" + codecClass);
					SequenceFileSpillWriter<ResultKeyType, ResultValueType> finalSpillWriter = new SequenceFileSpillWriter<ResultKeyType, ResultValueType>(
							remoteFileSystem, conf, shardDataFile, getKeyClass(), getValueClass(), null, codec,
							(short) 2);

					LOG.info(fqHostName + " Creating MergingSpillWriter at:" + hdfsAttemptPath + " for Query:"
							+ getQueryId());
					...
					try {
						...
						MergeSortSpillWriter<ResultKeyType, ResultValueType> mergingSpillWriter = new MergeSortSpillWriter<ResultKeyType, ResultValueType>(
								conf, finalSpillWriter, remoteFileSystem, mergeTemp, null, allocateRawComparator(),
								getKeyClass(), getValueClass(), null, null);

						try {
							LOG.info(fqHostName + " Slave Query Thread:Executing for Query:" + getQueryId());
							// execute the specific query object.
							long resultCount = executeShardedQuery(remoteFileSystem, conf, eventLoop, server,
									queryTempDir, hdfsAttemptPath, mergingSpillWriter,
									new QueryProgressCallback<DataType, ResultKeyType, ResultValueType>() {

										@Override
										public boolean updateProgress(
												final Query<DataType, ResultKeyType, ResultValueType> theQueryObject,
							...
							synchronized (Query.this) {
								if (_queryStatus.getStatus() != QueryStatus.Status.CANCELLED) {
									...
									for (FileStatus resultsFile : resultingFiles) {
										boolean success = remoteFileSystem.rename(resultsFile.getPath(), new Path(
												finalResultsPath, resultsFile.getPath().getName()));
										if (!success) {
											LOG.error(fqHostName + " Failed to Copy Result File:"
													+ resultsFile.getPath() + " to destination:" + finalResultsPath
													+ " for Query:" + getQueryId() + " Shard:"
													+ getRemoteQueryInfo().getShardId());
										} else {
											LOG.info(fqHostName + " Copied Results File:" + resultsFile.getPath()
													+ " to destination:" + finalResultsPath + " for Query:"
													+ getQueryId() + " Shard:" + getRemoteQueryInfo().getShardId());
							...
							if (eventLoop != null) {
								...
								eventLoop.setTimer(new Timer(0, false, new Timer.Callback() {
									...
									public void timerFired(Timer timer) {
										...
										LOG.info(fqHostName + " Query:" + getQueryId() + " Completed with:"
								...
								// execute directly from thread ... 
								LOG.info(fqHostName + " Query:" + getQueryId() + " Completed with:" + resultCount
										+ " results.");
								// and complete... 
					...
					if (eventLoop != null) {
						...
						eventLoop.setTimer(new Timer(0, false, new Timer.Callback() {
							...
							public void timerFired(Timer timer) {
								LOG.error(CCStringUtils.stringifyException(e));
								completionCallback.queryFailed(Query.this, "Query:" + getQueryId() + " Failed on Host:"
										+ fqHostName + " with Exception:" + CCStringUtils.stringifyException(e));
							}
						}));
					} else {
					...
					try {
						...
						LOG.error(fqHostName + " Failed to delete attempt directory:" + hdfsAttemptPath + " for Query:"
								+ getQueryId() + " Shard:" + getRemoteQueryInfo().getShardId());
					}
				}
	...
	public void startLocalQuery(final FileSystem fileSystem, final Configuration conf, final File tempFileDir,
			final EventLoop eventLoop, final QueryRequest<DataType, ResultKeyType, ResultValueType> queryRequest,
			final RemoteQueryCompletionCallback completionCallback) {
		...
		_queryThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					LOG.info("Client Query Thread for:" + getQueryId() + " creating temp file directory:"
							+ tempFileDir.getAbsolutePath());
					tempFileDir.mkdirs();

					LOG.info("Executing Local Query for Query:" + getQueryId());
					// execute local query 
					final long resultCount = executeAggregationQuery(fileSystem, conf, eventLoop, tempFileDir,
							queryRequest, new QueryProgressCallback<RPCStruct, WritableComparable, Writable>() {

								@Override
								public boolean updateProgress(
										Query<RPCStruct, WritableComparable, Writable> theQueryObject,
										float percentComplete) {
					...
					LOG.error("Query: " + getQueryId() + " Failed on executeLocal with Error:"
	...
	/** start client query thread**/
	public void startCacheQuery(final QueryController controller, final FileSystem fileSystem,
			final Configuration conf, final EventLoop eventLoop,
			final QueryRequest<DataType, ResultKeyType, ResultValueType> queryRequest,
			final QueryCompletionCallback<DataType, ResultKeyType, ResultValueType> completionCallback) {
		...
		_queryThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					// execute cache query 
					executeCacheQuery(fileSystem, conf, eventLoop, controller, queryRequest,
							new QueryCompletionCallback<DataType, ResultKeyType, ResultValueType>() {

								@Override
								public void queryComplete(
										final QueryRequest<DataType, ResultKeyType, ResultValueType> request,
										final QueryResult<ResultKeyType, ResultValueType> queryResult) {
								...
								public void queryFailed(
									...
									eventLoop.setTimer(new Timer(0, false, new Timer.Callback() {
										@Override
										public void timerFired(Timer timer) {
											LOG.info("Calling queryFailed on cacheRequest for Query:" + getQueryId()
													+ " Reason:" + reason);
					...
					LOG.error("Query: " + getQueryId() + " Failed on cacheQuery with Error:"
							+ CCStringUtils.stringifyException(e));
					final String error = CCStringUtils.stringifyException(e);
					eventLoop.setTimer(new Timer(0, false, new Timer.Callback() {
						@Override
	...
	public void startShardedQuery(QueryController controller, final QueryRequest request,
	...
	public void updateQueryStatus(QueryStatus statusUpdate) throws IOException {
		...
		if (slaveInfo != null) {
			...
			if (statusUpdate.getStatus() == QueryStatus.Status.ERROR
					&& slaveInfo._queryStatus.getStatus() != QueryStatus.Status.ERROR) {
				LOG.info("Slave:" + slaveInfo.getAssignedNode().getFullyQualifiedName() + " Reported Error:"
						+ statusUpdate.getOptErrorReason() + " for Query:" + getQueryId());
			} else if (statusUpdate.getStatus() == QueryStatus.Status.FINISHED
					&& slaveInfo._queryStatus.getStatus() != QueryStatus.Status.FINISHED) {
				LOG.info("Slave:" + slaveInfo.getAssignedNode().getFullyQualifiedName()
						+ " Reported FINISHED for Query:" + getQueryId() + " ResultCount:"
						+ statusUpdate.getOptResultCount());
			}
			try {
			...
			LOG.error("Query: " + getQueryId() + " Received Status Update from Unknown Shard:"
					+ statusUpdate.getShardId());
		}
	}
	...
	private void updateRemoteQueryStatus(final QueryRequest request) throws IOException {
			...
			if (shardStatus._queryStatus.getStatus() == QueryStatus.Status.PENDING) {
				...
				if (shardStatus._queryStatus.getLastAttemptTime() == 0
					...
					if (shardStatus.getQueryStatus().getAttempts() > MAX_RETRIES) {
						LOG.error("Failed to execute Query:" + getQueryId() + " for Shard:"
								+ shardStatus.getQueryStatus().getShardId());
					...
					if (shardStatus.getAssignedNode() == null) {
						String fqnHost = _queryController.getHost().mapShardToHostFQN(this,
								shardStatus.getQueryStatus().getShardId());
						if (fqnHost != null) {
							...
							if (dispatchPoint == null) {
								LOG.error("Failed to map fqnHost:" + fqnHost + " for shardId:"
										+ shardStatus.getQueryStatus().getShardId() + " and Query:" + getQueryId()
										+ " to valid Connection");
							}
						} else {
							LOG.error("Failed to map shardId:" + shardStatus.getQueryStatus().getShardId()
									+ " for Query:" + getQueryId());
						}
					...
					if (shardStatus._assignedNode != null) {
						LOG.info("Sending Remote Query to Slave:" + shardStatus._assignedNode.getFullyQualifiedName()
								+ " for Shard:" + shardStatus.getQueryStatus().getShardId() + " Query:" + getQueryId());
						try {
							...
							LOG.error("Remote RPC For Query:" + getQueryId() + " to Slave:"
									+ shardStatus.getAssignedNode().getFullyQualifiedName() + " Failed with Exception:"
									+ CCStringUtils.stringifyException(e));
				...
				*/
				...
				if (shardStatus.getQueryStatus().getAttempts() < MAX_RETRIES) {
					...
					LOG.info("Marking FAILED Query:" + getQueryId() + " for Retry. Attempts("
							+ shardStatus.getQueryStatus().getAttempts() + "). Last Error Was:"
							+ shardStatus._queryStatus.getOptErrorReason());
					continue;
				} else {
					failureReason += "\n";
		...
		if (completedCount + failedCount == _shardStatusMap.size()
				|| (completedCount == 1 && totalResultCount == 1 && isSingleRequestQuery())) {
	...
	public long getQueryId() {
		return getCommonQueryInfo().getQueryId();
	}
	...
	final void dispatchQueryToSlave(final ShardState shardStatus) throws IOException {
		...
		shardStatus
				.getAssignedNode()
				.getRemoteStub()
				.doQuery(shardStatus.getQueryInfo(),
						new OutgoingMessageContext.Callback<RemoteQueryInfo, QueryStatus>() {
							@Override
							public void requestComplete(OutgoingMessageContext<RemoteQueryInfo, QueryStatus> request) {
								...
								if (request.getStatus() != OutgoingMessageContext.Status.Success) {
									LOG.error("Query:" + getQueryId() + " To Slave:" + targetHostFQN
											+ " Failed with RPC Error:" + request.getStatus());
									shardStatus._queryStatus.setStatus(QueryStatus.Status.PENDING);
								} else {
									LOG.info("Query:" + getQueryId() + " To Slave:" + targetHostFQN
											+ " returned Status:" + request.getOutput().getStatus());
									try {
	...
	protected boolean cachedResultsAvailable(FileSystem remoteFS, Configuration conf, ClientQueryInfo clientQueryInfo)
			throws IOException {
	...
	protected void executeCacheQuery(FileSystem remoteFS, Configuration conf, EventLoop eventLoop,
			QueryController queryController, QueryRequest<DataType, ResultKeyType, ResultValueType> theClientRequest,
			QueryCompletionCallback<DataType, ResultKeyType, ResultValueType> callback) throws IOException {
		...
		QueryResultFileIndex<ResultKeyType, ResultValueType> index = new QueryResultFileIndex<ResultKeyType, ResultValueType>(
				remoteFS, indexFileName, getKeyClass(), getValueClass());
		...
		index.readPaginatedResults(remoteFS, conf, theClientRequest.getClientQueryInfo().getSortOrder(),
				theClientRequest.getClientQueryInfo().getPaginationOffset(), theClientRequest.getClientQueryInfo()
						.getPageSize(), resultOut);
	...
	protected SpillWriter<ResultKeyType, ResultValueType> createAggregatedResultSpillWriter(
			FileSystem remoteFileSystem, Configuration conf, Path mergeResultsPath,
			QueryResultFileIndex.PositionBasedIndexWriter indexWriter) throws IOException {

		return new SequenceFileSpillWriter<ResultKeyType, ResultValueType>(remoteFileSystem, conf, mergeResultsPath,
				getKeyClass(), getValueClass(), new QueryResultFileIndex.PositionBasedIndexWriter(remoteFileSystem,
						QueryResultFileIndex.getIndexNameFromBaseName(mergeResultsPath)), null, (short) 2);
	}

	private long _aggregationCompletionCount = 0L;

	/** run the local porition of this query (runs on master)**/
	protected long executeAggregationQuery(FileSystem remoteFileSystem, Configuration conf, EventLoop eventLoop,
			File tempFirDir, QueryRequest<DataType, ResultKeyType, ResultValueType> requestObject,
			final QueryProgressCallback progressCallback) throws IOException {
		...
		try {
			...
			if (!remoteFileSystem.exists(mergeResultsPath)) {
				...
				SpillWriter<ResultKeyType, ResultValueType> mergedFileSpillWriter = createAggregatedResultSpillWriter(
						remoteFileSystem, conf, mergeResultsPath, new QueryResultFileIndex.PositionBasedIndexWriter(
								remoteFileSystem, QueryResultFileIndex.getIndexNameFromBaseName(mergeResultsPath)));
				try {
					SequenceFileMerger<ResultKeyType, ResultValueType> merger = new SequenceFileMerger<ResultKeyType, ResultValueType>(
							remoteFileSystem, conf, parts, mergedFileSpillWriter, getKeyClass(), getValueClass(),
			...
			QueryResultFileIndex<ResultKeyType, ResultValueType> indexFile = new QueryResultFileIndex<ResultKeyType, ResultValueType>(
					remoteFileSystem, indexFileName, getKeyClass(), getValueClass());
	 ...
	 * **/
	public boolean requiresShardedQuery(FileSystem remoteFS, Configuration conf, QueryController controller,
			QueryRequest<DataType, ResultKeyType, ResultValueType> theClientRequest, int shardCount,
			ArrayList<QueryInputSplit> splitsOut) throws IOException {
	...
	protected abstract long executeShardedQuery(final FileSystem fileSystem, final Configuration conf,
			final EventLoop eventLoop, final QuerySlaveServer slaveInterface, final File tempFirDir,
			final Path hdfsTempDir, final MergeSortSpillWriter<ResultKeyType, ResultValueType> spillWriter,
	...
	public void shardedQueryComplete(FileSystem fileSystem, Configuration conf,
			QueryRequest<DataType, ResultKeyType, ResultValueType> theClientRequest, long resultCount)
			throws IOException {
	}
	...
	protected Path getHDFSQueryResultsPath() {
		return new Path(getCommonQueryInfo().getQueryResultPath());
	}

	protected Path getHDFSQueryTempPath() {
		return new Path(getCommonQueryInfo().getQueryTempPath());
	}
