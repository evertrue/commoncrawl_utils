...
public class QuerySlaveConnection implements RPCChannel.ConnectionCallback {
	...
	public QuerySlaveConnection(QueryController master, String hostName, int rpcPort) {
		...
		try {
			...
			LOG.error("Unable to Resolve Slave HostName:" + hostName + " Exception:"
					+ CCStringUtils.stringifyException(e));
			throw new RuntimeException("Unable to Resolve Slave HostName:" + hostName + " Exception:"
					+ CCStringUtils.stringifyException(e));
		}
	...
	public void connect() throws IOException {
		...
		// initialize channel ... 
		_channel = new RPCChannel(_controller.getHost().getEventLoop(), null, _controller.getHost()
				.getOutgoingInterface(), _hostAddress, this);
		_channel.open();
	...
	public QueryServerSlave.AsyncStub getRemoteStub() {
		return _slaveService;
	}

	void enableHeartbeats() {
		_ignoreHeartbeats = false;
	}

	void disableHeartbeats() {
		_ignoreHeartbeats = true;
	}

	boolean areHeartbeatsDisabled() {
		return _ignoreHeartbeats;
	}
	...
	private void slaveOnline() {
		...
		try {
			...
			_slaveService.initializeQuerySlave(_controller.getBaseConfigForSlave(getFullyQualifiedName()),
					new Callback<BaseConfig, SlaveStatus>() {
