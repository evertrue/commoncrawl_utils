...
public class QueryController {
	 ...
	 */
	public QueryController(QueryControllerHost host, FileSystem remoteFS, Configuration conf, Path hdfsResultsDir,
			Path hdfsQueryHistoryDir, File localTempDir, int maxConcurrentQueries) {
			...
			for (QueryStatus queryStatus : slaveStatus.getQueryStatus()) {
				...
				if (request != null) {
					...
					try {
						...
						LOG.error("Error Updating QueryStatus for Query:" + request.getSourceQuery().getQueryId()
								+ " Slave:" + fqnHostName + " Error:" + CCStringUtils.stringifyException(e));
					}
				} else {
	...
	@SuppressWarnings("unchecked")
	public <DataType extends RPCStruct, KeyType extends WritableComparable, ValueType extends Writable> BlockingQueryResult<KeyType, ValueType> blockingQueryRequest(
			final Query<DataType, KeyType, ValueType> queryObject, final ClientQueryInfo queryInfo) throws IOException {

		final LinkedBlockingQueue<BlockingQueryResult<KeyType, ValueType>> queue = new LinkedBlockingQueue<BlockingQueryResult<KeyType, ValueType>>(
				1);
		...
		_host.getEventLoop().setTimer(new Timer(0, false, new Timer.Callback() {
			...
			public void timerFired(Timer timer) {
				try {
					queueClientQueryRequest(queryObject, queryInfo,
							new QueryCompletionCallback<DataType, KeyType, ValueType>() {

								@Override
								public void queryComplete(QueryRequest<DataType, KeyType, ValueType> request,
										QueryResult<KeyType, ValueType> queryResult) {
									LOG.info("Recevied QueryComplete for query:"
											+ request.getSourceQuery().getQueryId() + " Object:"
											+ request.getSourceQuery());
									BlockingQueryResult<KeyType, ValueType> result = new BlockingQueryResult<KeyType, ValueType>(
											queryResult);
									try {
										LOG.info("Queing response for Query:" + request.getSourceQuery().getQueryId()
												+ " Object:" + request.getSourceQuery());
										queue.put(result);
								...
								@Override
								public void queryFailed(QueryRequest<DataType, KeyType, ValueType> request,
										String reason) {
									LOG.info("Received queryFailed for request:"
											+ request.getSourceQuery().getQueryId() + " Object:"
											+ request.getSourceQuery());
									BlockingQueryResult<KeyType, ValueType> result = new BlockingQueryResult<KeyType, ValueType>(
											reason);
	...
	long locateMatchingQueryInRequestCollection(Query queryObject, Collection<QueryRequest> collection) {
		...
		for (QueryRequest request : collection) {
			...
			LOG.info("Matching " + queryCanonicalId.e0 + ":" + queryCanonicalId.e1 + " Against "
					+ requestCanonicalId.e0 + ":" + requestCanonicalId.e1);
			if (((long) requestCanonicalId.e0) == (long) queryCanonicalId.e0
					&& ((long) requestCanonicalId.e1) == (long) queryCanonicalId.e1) {
	...
	@SuppressWarnings("unchecked")
	<DataType extends RPCStruct, KeyType extends WritableComparable, ValueType extends Writable> void queueClientQueryRequest(
			Query<DataType, KeyType, ValueType> queryObject, ClientQueryInfo theClientRequest,
			QueryCompletionCallback<DataType, KeyType, ValueType> callback) throws IOException {

		// set query info
		queryObject.setClientQueryInfo(theClientRequest);

		// now check cache for persistent query cache info ...
		PersistentQueryInfo persistentQueryInfo = getPersistentQueryInfo(queryObject.getCanonicalQueryId(),
				theClientRequest.getMaxAge());
		...
		QueryRequest<DataType, KeyType, ValueType> clientQueryObj = new QueryRequest<DataType, KeyType, ValueType>(
				queryObject, theClientRequest, callback);
	...
	private void publishBindings(Query queryObject) throws IOException {
		...
		Path canonicalToIdBinding = new Path(_hdfsQueryHistoryDir, canonicalIdAndQueryIdToFilename(queryCanonicalId,
				queryObject.getQueryId()));
		if (!_remoteFS.exists(canonicalToIdBinding)) {

			LOG.info("Binding CanonicalQuery Id:[" + queryCanonicalId.e0 + ":" + queryCanonicalId.e1 + "] to QueryId:"
					+ queryObject.getQueryId() + " via path:" + canonicalToIdBinding);
			...
			for (QueryRequest request : requests) {
				...
				try {
					...
					if (_activeRemoteOrLocalQueries.get(request.getSourceQuery().getQueryId()) != null) {
						...
						if (request.getSourceQuery().cachedResultsAvailable(fileSystem, _conf,
								request.getClientQueryInfo())) {
							// remove from queued 
						...
						else if (request.getSourceQuery().requiresShardedQuery(fileSystem, _conf, this, request,
								getHost().getShardCountForQuery(request.getSourceQuery()), requiredSplits)) {
							if (remoteDispatchSlotsAvail != 0) {
								_queuedClientQueries.remove();
								// ok, we need at least on shard to run on ... 
								if (requiredSplits.size() == 0) {
									LOG.error("Query:" + request.getSourceQuery().getQueryId()
											+ " FAILED WITH EMPTY REQUIRED SHARDS LIST");
					...
					_queuedClientQueries.remove(request);
					// log error 
					LOG.error("Client Request:" + request.getClientQueryInfo().getClientQueryId()
	...
	private void deactivateRequest(QueryRequest request, RunState newRunState) {
		LOG.info("DeActivating Query:" + request.getSourceQuery().getQueryId() + " RunState:"
				+ request.getRunState().toString());
		...
		if (lastRunState == RunState.RUNNING_REMOTE || lastRunState == RunState.RUNNING_LOCAL
				|| lastRunState == RunState.ERROR) {
			_activeRemoteOrLocalQueries.remove(request.getSourceQuery().getQueryId());
			...
			if (request.getRunState() == RunState.IDLE) {
				LOG.info("Marking Query:" + request.getSourceQuery().getQueryId() + " SeqNo:" + request.getSequenceNo()
						+ " for Requeue");
				requeue = true;
			}
		}
		...
		LOG.info("** Deleting Temp File for Query:" + request.getSourceQuery().getQueryId() + " At:"
				+ queryTempFile.getAbsolutePath());
		FileUtils.recursivelyDeleteFile(queryTempFile);
		LOG.info("** Deleting HDFS Temp Dir:" + hdfsQueryTempDir + " for Query:"
		...
		if (request.getRunState() == RunState.ERROR) {
			...
			LOG.info("** Query FAILED. Deleting HDFS Working Dir:" + hdfsWorkingDir + " for Query:"
					+ request.getSourceQuery().getQueryId());
			try {
					...
					new RemoteQueryCompletionCallback() {
						...
						public void queryComplete(Query query, long resultCount) {
							...
							try {
								...
								if (resultCount > 0) {
									LOG.info("Remote Query:" + request.getSourceQuery().getQueryId() + " returned:"
											+ resultCount + " results");
									// deactive request first ...
									requeueRequest((QueryRequest) query.getContextObject());
								} else {

									LOG.info("Query:" + request.getSourceQuery().getQueryId()
								...
								String error = "Query: " + request.getSourceQuery().getQueryId()
										+ " Failed with Exception:" + CCStringUtils.stringifyException(e);
								LOG.error(error);
						...
						public void queryFailed(Query query, final String reason) {
							LOG.info("Recevied QueryFailed for Query:" + request.getSourceQuery().getQueryId()
									+ " Reason:" + reason);
	...
	private void runLocalQuery(final QueryRequest request) {
		...
		request.getSourceQuery().startLocalQuery(_remoteFS, _conf,
				getTempDirForQuery(request.getSourceQuery().getQueryId()), getHost().getEventLoop(), request,
				new RemoteQueryCompletionCallback() {
					...
					public void queryComplete(Query query, long resultCount) {
						LOG.info("Recevied QueryComplete for Query:" + request.getSourceQuery().getQueryId());
						if (resultCount > 0) {
							LOG.info("Local Query:" + request.getSourceQuery().getQueryId() + " returned:"
									+ resultCount + " results");
							// requeue request ...
							requeueRequest((QueryRequest) query.getContextObject());
						} else {
					...
					@Override
					public void queryFailed(Query query, final String reason) {
						LOG.info("Recevied QueryFailed for Query:" + request.getSourceQuery().getQueryId() + " Reason:"
								+ reason);
						deactivateRequest((QueryRequest) query.getContextObject(), RunState.ERROR);
						request.getCompletionCallback().queryFailed(request, reason);
					}
	...
	private void runCacheQuery(QueryRequest request) {
		...
		request.getSourceQuery().startCacheQuery(this, _remoteFS, _conf, getHost().getEventLoop(), request,
	...
	private PersistentQueryInfo getPersistentQueryInfo(Pair<Long, Long> queryCanonicalId, long maxAge)
			throws IOException {
		// construct pattern 
